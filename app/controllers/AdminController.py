...
class AdminController:
    ...
    def _register_routes(self):
        # Dashboard
        self.blueprint.add_url_rule("/",                    view_func=self.admin,         methods=["GET"])
        # Users
        self.blueprint.add_url_rule("/users",               view_func=self.getUsers,      methods=["GET"])
        self.blueprint.add_url_rule("/users/<int:user_id>", view_func=self.adminDelete,   methods=["DELETE"])
        self.blueprint.add_url_rule("/users/<int:id>/role", view_func=self.updateRole,    methods=["PUT"])
        # Repas
        self.blueprint.add_url_rule("/repas",               view_func=self.ajouterRepas,  methods=["POST"])
        self.blueprint.add_url_rule("/repas/<int:id>",      view_func=self.deleteRepas,   methods=["DELETE"])
        self.blueprint.add_url_rule("/repas/<int:id>",      view_func=self.modifierRepas, methods=["PUT"])
    ...
    def admin(self):
        ...
        return jsonify({
            ...
            "repas":     [r.to_dict() for r in rs.getRepasAll()],
            "users":     [u.to_dict() for u in us.getUsers()]
        })
    ...
    def getUsers(self):
        ...
        if role_filter and role_filter != 'tous':
            users = us.findByRole(role_filter)
        else:
            users = us.getUsers()
    ...
    def adminDelete(self, user_id):
        us = UserService()
        us.deleteUser(user_id)
        return jsonify({"success": True, "message": f"Utilisateur #{user_id} supprimé"})
    ...
    def updateRole(self, id):
        ...
        us.updateByRole(id, nouveau_role)
        return jsonify({"success": True, "message": f"Utilisateur #{id} mis à jour : {nouveau_role}"})

    def ajouterRepas(self):
        rs          = RepasService()
        nom         = request.form.get('nom')
        categorie   = request.form.get('categorie')
        prix        = request.form.get('prix')
        ...
        rs.ajouterRepas(nom, description, categorie, prix, statut, quantite, filename)
        return jsonify({"success": True, "message": f"{nom} ajouté"}), 201
    ...
    @limiter.limit("10 per minute")
    def modifierRepas(self, id):
        rs   = RepasService()
        data = request.json or {}

        n = data.get('nom')
        c = data.get('categorie')
        p = data.get('prix')
        q = data.get('quantite')
        s = data.get('statut')
        d = data.get('description')

        if s not in ['disponible', 'indisponible']:
            return jsonify({"error": "Statut invalide"}), 400

        if c not in ['Entrées', 'Plats principaux', 'Desserts']:
            return jsonify({"error": "Catégorie invalide"}), 400

        rs.updateRepas(id, n, d, c, p, s, q)
        return jsonify({"success": True, "message": "Repas modifié"})
